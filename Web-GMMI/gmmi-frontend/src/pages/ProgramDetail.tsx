...
import { getPrograms, exportExcel, exportWord } from '../services/program.service';
...
const ProgramDetailPage: React.FC = () => {
    ...
    const [exportLoading, setExportLoading] = useState<'excel' | 'word' | null>(null);
            ...
            <section className="relative pt-32 pb-48 overflow-hidden bg-gmmi-navy">
                ...
                <div className="container mx-auto px-6 relative z-10">
                    <motion.div
                        ...
                        className="space-y-8"
                    >
                        <Link to="/program" className="inline-flex items-center gap-2 text-gmmi-gold/80 hover:text-gmmi-gold transition-colors text-sm font-bold uppercase tracking-[0.3em]">
                        ...
                        <div className="flex flex-col md:flex-row justify-between items-end gap-8">
                            <div className="space-y-4">
                                <div className="flex items-center gap-3 text-gmmi-gold">
                                    ...
                                    <span className="text-xs font-black uppercase tracking-[0.4em]">Detail Rencana Kerja</span>
                                </div>
                            ...
                            <div className="flex gap-4">
                                <Button
                                    ...
                                    startContent={!exportLoading && <Download size={20} className="text-gmmi-gold" />}
                                >
                                    Export Excel
                                </Button>
                                <Button
            ...
            <main className="container mx-auto px-6 -mt-32 relative z-20 pb-32">
                <Card className="p-8 rounded-[3rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.1)] border-none bg-white">
                    ...
                    >
                        ...
                        <TableBody
                            emptyContent={loading ? null : "Belum ada data program untuk bidang ini."}
                            items={programs}
                        >
                            {loading ? (
                                Array(5).fill(0).map((_, i) => (
                                    <TableRow key={i}>
                                        <TableCell><Skeleton className="w-4/5 h-4 rounded-full" /></TableCell>
                                        <TableCell><Skeleton className="w-1/2 h-4 rounded-full" /></TableCell>
                                        <TableCell><Skeleton className="w-full h-4 rounded-full" /></TableCell>
                                        <TableCell><Skeleton className="w-1/2 h-4 rounded-full" /></TableCell>
                                        <TableCell><Skeleton className="w-1/3 h-4 rounded-full" /></TableCell>
                                        <TableCell><Skeleton className="w-2/3 h-4 rounded-full" /></TableCell>
                                    </TableRow>
                                ))
                            ) : (
                                (item) => (
                                    <TableRow key={item.id} className="border-b border-neutral-50 last:border-none hover:bg-neutral-50/50 transition-colors">
                                        <TableCell>
                                            <span className="font-bold text-gmmi-navy text-lg tracking-tight">{item.nama_program}</span>
                                        </TableCell>
                                        <TableCell>
                                            <Chip variant="flat" className="bg-gmmi-navy/5 text-gmmi-navy font-bold text-[10px] uppercase px-3">
                                        ...
                                        <TableCell>
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-xs font-bold uppercase">{item.waktu_pelaksanaan}</span>
                                            </div>
                                        </TableCell>
                                        <TableCell>
                                            <span className="font-black text-gmmi-navy">
                                                {Number(item.rencana_biaya) > 0 ? `Rp ${Number(item.rencana_biaya).toLocaleString('id-ID')}` : "-"}
                                            </span>
                                        </TableCell>
                                        <TableCell>
                ...
                <div className="mt-16 bg-neutral-50 rounded-[2.5rem] p-12 flex flex-col md:flex-row justify-between items-center gap-8 border border-neutral-100">
                    <div className="flex items-center gap-6">
                        ...
                        <div className="space-y-1">
                            <h4 className="text-xl font-bold text-gmmi-navy">Informasi Tambahan</h4>
                            <p className="text-neutral-500 font-light">Seluruh anggaran yang tertera merupakan estimasi awal rencana kerja tahunan.</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-10">
                        <div className="text-center">
                            <p className="text-[10px] font-black uppercase tracking-widest text-neutral-400 mb-1">Total Program</p>
                            <p className="text-3xl font-black text-gmmi-navy">{programs.length}</p>
                        </div>
                        <Divider orientation="vertical" className="h-12" />
                        <div className="text-center">
                            <p className="text-[10px] font-black uppercase tracking-widest text-neutral-400 mb-1">Total Anggaran</p>
                            <p className="text-3xl font-black text-gmmi-gold">
                                Rp {programs.reduce((acc, curr) => acc + Number(curr.rencana_biaya || 0), 0).toLocaleString('id-ID')}
                            </p>
                        </div>
                    </div>
                </div>
