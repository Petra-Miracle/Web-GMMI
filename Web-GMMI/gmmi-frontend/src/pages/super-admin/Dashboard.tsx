    ...
    return (
        <div className="space-y-8">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-gmmi-navy">Super Admin Dashboard</h1>
                    <p className="text-gray-500">Selamat datang, {user?.name || 'Administrator'}. Berikut statistik terkini sistem GMMI.</p>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <Card className="shadow-md hover:shadow-lg transition-shadow bg-gradient-to-br from-indigo-500 to-indigo-600 text-white border-none">
                            <CardBody className="p-6">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <p className="text-xs font-bold uppercase tracking-widest opacity-80">Total Jemaat</p>
                                        <h3 className="text-3xl font-black mt-2">{displayTotalJemaat}</h3>
                ...
                <Card className="shadow-md hover:shadow-lg transition-shadow bg-gradient-to-br from-cyan-500 to-cyan-600 text-white border-none">
                    <CardBody className="p-6">
                        <div className="flex justify-between items-start">
                            <div>
                                <p className="text-xs font-bold uppercase tracking-widest opacity-80">Total Sektor</p>
                                <h3 className="text-3xl font-black mt-2">{stats?.totalSectors || 0}</h3>
                ...
                <Card className="shadow-md hover:shadow-lg transition-shadow bg-gradient-to-br from-violet-500 to-violet-600 text-white border-none">
                    <CardBody className="p-6">
                        <div className="flex justify-between items-start">
                            <div>
                                <p className="text-xs font-bold uppercase tracking-widest opacity-80">Total Admin</p>
                ...
                <Card className="shadow-md hover:shadow-lg transition-shadow bg-gradient-to-br from-slate-700 to-slate-800 text-white border-none">
                    <CardBody className="p-6">
                        <div className="flex justify-between items-start">
                            <div>
                                <p className="text-xs font-bold uppercase tracking-widest opacity-80">Status Sistem</p>
                ...
                <div className="space-y-6">
                    <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-4">
                        <div>
                            ...
                            <p className="text-sm text-gray-500">Analisis sebaran data jemaat berdasarkan sektor dan kategori.</p>
                        </div>
                        <div className="w-full md:w-72">
                            ...
                            >
                                <SelectItem key="all" startContent={<Database size={16} />}>Semua Sektor</SelectItem>
                                {(stats.sectorsList || []).map((sec: any) => (
                                    <SelectItem key={sec.nama_sektor} startContent={<Map size={16} />}>
                                        {sec.nama_sektor}
                                    </SelectItem>
                                ))}
                            </Select>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        {(() => {
                            const data = (stats.educationStats || [])
                                .filter((item: any) => selectedSector === "all" || item.sector === selectedSector)
                                .reduce((acc: any[], curr: any) => {
                                    const existing = acc.find(x => x.name === curr.education);
                                    if (existing) {
                                        existing.value += parseInt(curr.count);
                                    } else {
                                        acc.push({ name: curr.education || 'Tidak Diketahui', value: parseInt(curr.count) });
                                    }
                                    return acc;
                                }, [])
                            ...
                            return (
                                <Card className="shadow-lg border-none bg-white/80 backdrop-blur-md overflow-visible relative group">
                                    ...
                                    <CardBody className="p-6">
                                        <div className="mb-6">
                                            <h4 className="font-bold text-gray-800 text-lg">Pendidikan Terakhir</h4>
                                            <p className="text-xs text-gray-400">Distribusi tingkat pendidikan</p>
                                        </div>
                                                ...
                                                <ResponsiveContainer width="100%" height="100%">
                                                    <PieChart>
                                                        ...
                                                        <Legend
                                                            ...
                                                            iconType="circle"
                                                            iconSize={8}
                                                            formatter={(value) => <span className="text-xs font-semibold text-gray-500 ml-1">{value}</span>}
                                                        />
                                                    </PieChart>
                                                </ResponsiveContainer>
                                            ) : (
                        ...
                        {(() => {
                            const data = (stats.kategorialStats || [])
                                ...
                                .reduce((acc: any[], curr: any) => {
                                    ...
                                    if (existing) {
                                        ...
                                        acc.push({ name: curr.category || 'Lainnya', value: parseInt(curr.count) });
                                    }
                                    return acc;
                                }, [])
                                .sort((a: any, b: any) => b.value - a.value);
                            ...
                            return (
                                <Card className="shadow-lg border-none bg-white/80 backdrop-blur-md overflow-visible relative group">
                                    ...
                                    <CardBody className="p-6">
                                        <div className="mb-6">
                                            <h4 className="font-bold text-gray-800 text-lg">Kategorial</h4>
                                            <p className="text-xs text-gray-400">Pembagian kelompok usia</p>
                                        </div>
                                        <div className="h-64 w-full relative z-10">
                                            {data.length > 0 ? (
                                                <ResponsiveContainer width="100%" height="100%">
                        ...
                        {(() => {
                            const data = (stats.sakramenStats || [])
                                ...
                                .reduce((acc: any[], curr: any) => {
                                    ...
                                    if (existing) {
                                        ...
                                        acc.push({ name: curr.sacrament || 'Lainnya', value: parseInt(curr.count) });
                                    }
                                    return acc;
                                }, [])
                                .sort((a: any, b: any) => b.value - a.value);
                            ...
                            return (
                                <Card className="shadow-lg border-none bg-white/80 backdrop-blur-md overflow-visible relative group">
                                    ...
                                    <CardBody className="p-6">
                                        <div className="mb-6">
                                            <h4 className="font-bold text-gray-800 text-lg">Sakramen</h4>
                                            <p className="text-xs text-gray-400">Status penerimaan sakramen</p>
                                        </div>
                                        <div className="h-64 w-full relative z-10">
                                            {data.length > 0 ? (
                                                <ResponsiveContainer width="100%" height="100%">
                    ...
                    <Card className="shadow-lg border-none bg-white p-8 rounded-3xl mt-6">
                        <CardBody className="p-0">
                            <div className="flex justify-between items-center mb-8">
                                <div>
                                    <h4 className="font-black text-xl text-gray-800 uppercase tracking-tighter">Sebaran Jemaat per Sektor</h4>
                                    <p className="text-sm text-gray-400 font-medium">Kepadatan jiwa di setiap wilayah pelayanan.</p>
                                </div>
                            ...
                            <div className="space-y-6">
                                {(() => {
                                    const sectorCounts = (stats.educationStats || []).reduce((acc: any, curr: any) => {
                                        acc[curr.sector] = (acc[curr.sector] || 0) + parseInt(curr.count);
                                        return acc;
                                    }, {});
                                    ...
                                    const maxCount = Math.max(...Object.values(sectorCounts) as number[], 1);

                                    return sortedSectors.map(([sector, count]: any, idx: number) => (
                                        <div key={idx} className="group">
                                            <div className="flex justify-between items-end mb-2">
                                                <span className="font-black text-xs text-gray-700 uppercase tracking-widest">{sector}</span>
                                                <span className="font-mono text-xs font-bold text-blue-900 bg-blue-50 px-2 py-0.5 rounded-md">{count} Jiwa</span>
                                            </div>
                                ...
                                {(!stats.educationStats || stats.educationStats.length === 0) && (
                                    <p className="text-center text-gray-400 py-4 italic">Belum ada data persebaran jemaat.</p>
                                )}
                            </div>
                        </CardBody>
                    </Card>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
                ...
                <Card className="shadow-md hover:shadow-lg transition-shadow bg-white border border-gray-100">
                    <CardBody className="flex flex-row items-center gap-4 p-6">
                        ...
                        <div>
                            <p className="text-sm text-gray-500 font-medium">Total Pewartaan</p>
                            <h3 className="text-2xl font-bold text-gmmi-navy">
                                {stats?.totalPewartaan || 0}
                            </h3>
                            <p className="text-xs text-green-600 font-medium">{stats?.activePewartaan || 0} Aktif</p>
                        </div>
                    </CardBody>
                </Card>
                ...
                <Card className="shadow-md hover:shadow-lg transition-shadow bg-white border border-gray-100">
                    <CardBody className="flex flex-row items-center gap-4 p-6">
                        ...
                        <div>
                            <p className="text-sm text-gray-500 font-medium">Jumlah Program Kerja Tahun ini</p>
                            <h3 className="text-2xl font-bold text-gmmi-navy">
                ...
                <Card className="shadow-md hover:shadow-lg transition-shadow bg-white border border-gray-100">
                    <CardBody className="flex flex-row items-center gap-4 p-6">
                        ...
                        <div>
                            <p className="text-sm text-gray-500 font-medium">Total Renungan</p>
                            <h3 className="text-2xl font-bold text-gmmi-navy">
            ...
            <div className="space-y-6">
                <div className="flex justify-between items-center">
                    <h3 className="font-bold text-xl text-gmmi-navy">Ringkasan Keuangan</h3>
                        ...
                        {user?.role?.includes('super') && (
                            <>
                                <Button
                                    ...
                                    onPress={() => {
                                        // Direct export call
                                        import('../../services/keuangan.service').then(m => m.exportTransactions());
                                    }}
                                >
                                    Export Excel
                                </Button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <Card className="bg-emerald-50 border-emerald-100 shadow-sm border">
                        <CardBody className="p-4 flex flex-row items-center gap-4">
                            ...
                            <div>
                                <p className="text-[10px] uppercase font-black text-emerald-600 tracking-widest">Total Pemasukan</p>
                                <h4 className="text-xl font-black text-emerald-800">Rp {(stats?.income || 0).toLocaleString('id-ID')}</h4>
                            </div>
                        </CardBody>
                    </Card>
                    <Card className="bg-rose-50 border-rose-100 shadow-sm border">
                        <CardBody className="p-4 flex flex-row items-center gap-4">
                            ...
                            <div>
                                <p className="text-[10px] uppercase font-black text-rose-600 tracking-widest">Total Pengeluaran</p>
                                <h4 className="text-xl font-black text-rose-800">Rp {(stats?.expense || 0).toLocaleString('id-ID')}</h4>
                            </div>
                        </CardBody>
                    </Card>
                    <Card className="bg-blue-50 border-blue-100 shadow-sm border">
                        <CardBody className="p-4 flex flex-row items-center gap-4">
                            ...
                            <div>
                                <p className="text-[10px] uppercase font-black text-blue-600 tracking-widest">Saldo Bersih</p>
                                <h4 className="text-xl font-black text-blue-800">Rp {((stats?.income || 0) - (stats?.expense || 0)).toLocaleString('id-ID')}</h4>
                            </div>
                        </CardBody>
                    </Card>
                </div>
                        ...
                        <table className="w-full text-left border-collapse min-w-[600px]">
                            <thead>
                                <tr className="bg-gray-50/50">
                                    ...
                                    <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400 text-right">Masuk</th>
                                    <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400 text-right">Keluar</th>
                                </tr>
                            </thead>
                                ...
                                {stats?.recentFinance?.length > 0 ? (
                                    stats.recentFinance.map((row: any) => (
                                        <tr key={row.id} className="hover:bg-gray-50/50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-xs text-slate-600 font-medium">{row.keterangan}</td>
                                    ...
                                    <tr>
                                        <td colSpan={4} className="p-8 text-center text-gray-400 text-xs">Belum ada transaksi tercatat.</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <Card className="lg:col-span-2 shadow-sm border border-gray-100 bg-white">
                    <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                        <h3 className="font-bold text-lg text-gmmi-navy">Aktivitas Terkini</h3>
                        <Button aria-label="Lihat semua aktivitas" as={Link} to="/super-admin/arsip" size="sm" variant="light" color="primary">Lihat Semua</Button>
                    </div>
                    <CardBody className="p-0">
                        {stats?.recentActivities?.length > 0 ? (
                            <div className="divide-y divide-gray-50">
                                {stats.recentActivities.map((activity: any, index: number) => (
                                    <div key={index} className="p-4 hover:bg-gray-50 transition-colors flex items-center gap-4 border-l-4 border-transparent hover:border-gmmi-navy/30">
                                        <div className={`p-2.5 rounded-xl shadow-sm ${activity.type === 'keuangan' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' :
                                            activity.type === 'jemaat' ? 'bg-blue-50 text-blue-600 border border-blue-100' :
                                                activity.type === 'program' ? 'bg-amber-50 text-amber-600 border border-amber-100' :
                                                    activity.type === 'admin' ? 'bg-indigo-50 text-indigo-600 border border-indigo-100' :
                                                        'bg-slate-50 text-slate-600 border border-slate-100'
                                            }`}>
                                            {activity.type === 'keuangan' ? <TrendingUp size={18} /> :
                                                activity.type === 'jemaat' ? <Users size={18} /> :
                                                    activity.type === 'program' ? <ListTodo size={18} /> :
                                                        activity.type === 'admin' ? <ShieldCheck size={18} /> :
                                                            <Megaphone size={18} />}
                                        </div>
                                        <div className="flex-1">
                                            <p className="text-sm font-bold text-gray-800 leading-snug">{activity.title}</p>
                                            <p className="text-[10px] text-gray-400 font-medium uppercase tracking-wider mt-0.5">{activity.type}</p>
                                        </div>
                                        <div className="text-right">
                                            <span className="text-xs font-bold text-gmmi-navy bg-gray-50 px-2 py-1 rounded-md">{activity.time}</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                <div className="space-y-6">
                    ...
                    <Card className="shadow-sm border border-gray-100 bg-white">
                        <CardBody className="p-6">
                            <h3 className="font-bold text-lg text-gmmi-navy mb-4">Agenda Terdekat</h3>
                            {stats?.upcomingAgenda && stats.upcomingAgenda.length > 0 ? (
                                    ...
                                    {stats.upcomingAgenda.map((item: any) => {
                                        ...
                                        return (
                                            <div key={item.id} className="flex gap-3 items-start">
                                                ...
                                                <div>
                                                    <h4 className="font-semibold text-gray-800 text-sm">{item.kegiatan}</h4>
                                                    <p className="text-xs text-gray-500">
                                                        {item.jam_mulai?.substring(0, 5)} WIB • {item.lokasi || 'GMMI Pusat'}
                                                    </p>
                                                </div>
                                            </div>
                                        );
                                ...
                                <p className="text-gray-500 text-sm">Belum ada agenda terdekat.</p>
                            )}
                        </CardBody>
                    </Card>
                </div>
