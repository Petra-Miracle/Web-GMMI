...
const Renungan: React.FC = () => {
    ...
    const filteredRenungan = renunganList.filter(item =>
                ...
                <Card className="p-4 md:p-6 rounded-[2.5rem] bg-white/80 backdrop-blur-xl shadow-2xl shadow-gmmi-navy/5 border border-white/50 mb-16 max-w-5xl mx-auto">
                    <div className="flex flex-col md:flex-row gap-4 items-center">
                        <Input
                            ...
                            startContent={<Search className="text-gmmi-gold ml-2" size={20} />}
                            value={searchTerm}
                            ...
                            variant="flat"
                            classNames={{
                                inputWrapper: "bg-neutral-50 hover:bg-neutral-100 transition-colors h-14"
                            }}
                            className="w-full"
                        />
                        <div className="flex items-center gap-3 px-6 py-2 bg-gmmi-navy text-white rounded-full shrink-0 h-14 shadow-lg shadow-gmmi-navy/20">
                            ...
                            <span className="font-bold text-sm whitespace-nowrap">{filteredRenungan.length} Renungan</span>
                        </div>
                    </div>
                </Card>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
                    {loading ? (
                        Array(6).fill(0).map((_, i) => (
                            <Card key={i} className="rounded-[2.5rem] overflow-hidden bg-white border border-neutral-100 shadow-sm h-[420px]">
                                    ...
                                    <div className="relative h-[240px] overflow-hidden shrink-0">
                                        {item.gambar ? (
                                            ...
                                            <div className="w-full h-full bg-gmmi-navy flex items-center justify-center relative overflow-hidden">
                                                ...
                                                <Quote size={48} className="text-gmmi-gold relative z-10" />
                                            </div>
                                        )}
                        ...
                        <div className="col-span-full py-32 text-center bg-white rounded-[3rem] border border-dashed border-neutral-200">
                            ...
                            <h3 className="text-xl font-bold text-gmmi-navy mb-2">Renungan tidak ditemukan</h3>
                            <p className="text-slate-400">Coba cari dengan kata kunci lain.</p>
                        </div>
                    )}
                </div>
            </main>
