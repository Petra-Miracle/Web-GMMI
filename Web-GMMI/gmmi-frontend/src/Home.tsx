...
const HeroCarousel: React.FC<{ slides: CarouselSlide[] }> = ({ slides }) => {
    ...
    return (
        <section className="relative h-screen w-full overflow-hidden bg-slate-900 shadow-2xl group">
            ...
            <div className="absolute z-40 flex items-center gap-4 xl:gap-6
...
const SejarahSection: React.FC<{ sejarahList: Sejarah[] }> = ({ sejarahList = [] }) => {
    // Determine content to display
    const data = Array.isArray(sejarahList) && sejarahList.length > 0 ? sejarahList[0] : null;
    ...
    const year = data?.tanggal_peristiwa ? new Date(data.tanggal_peristiwa).getFullYear() : "1999";
    const title = data?.judul || "Melayani dengan Kasih & Iman";
    const description = data?.deskripsi || "GMMI lahir dari kerinduan untuk menyediakan wadah persekutuan yang berakar kuat pada Alkitab dan memiliki kepedulian sosial yang nyata bagi sesama.";
    ...
    return (
        <section id="sejarah" className="relative py-32 overflow-hidden bg-neutral-50/50">
            {/* Decorative Elements */}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
                    ...
                    >
                        <div className="relative z-10 rounded-[3rem] overflow-hidden shadow-2xl border-8 border-white">
                            <img
                                ...
                                onError={(e) => {
                                    ...
                                    if (!target.src.includes("placeholder")) {
                                        target.src = "https://placehold.co/600x400?text=GMMI+History";
                                    }
                                }}
                            ...
                            <div className="absolute bottom-10 left-10 text-white">
                                <p className="text-5xl font-serif font-bold text-gmmi-gold">{year}</p>
                                <p className="text-sm font-black uppercase tracking-[0.3em] opacity-80">Tahun Peristiwa</p>
                            </div>
                        </div>
                        ...
                        <div className="hidden md:block absolute -bottom-10 -right-10 z-20">
                            ...
                            >
                                <div className="flex items-center gap-6">
                                    ...
                                    <div>
                                        <p className="text-3xl font-bold text-gmmi-navy tracking-tight">{new Date().getFullYear() - 1999}+</p>
                                        <p className="text-[10px] font-black uppercase tracking-widest text-neutral-400">Tahun Melayani</p>
                                    </div>
                                </div>
                            </motion.div>
                        </div>
                    ...
                    >
                        ...
                        <div className="space-y-6 text-lg text-neutral-600 font-light leading-relaxed">
                            ...
                            {!data && (
                                <motion.p variants={sejarahItemVariants}>
                                    Sejak didirikan pada tahun 1999, GMMI terus berkomitmen untuk membangun jemaat yang militan dalam iman namun inklusif dalam pelayanan. Kami percaya bahwa setiap pribadi dipanggil untuk menjadi garam dan terang di tengah dunia.
                                </motion.p>
                            )}
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-8 pt-6">
                            <motion.div variants={sejarahItemVariants} className="space-y-3">
                                ...
                                <h4 className="font-bold text-gmmi-navy text-xl uppercase tracking-tighter">Visi Utama</h4>
                                <p className="text-sm text-neutral-500 font-medium">Menjadi gereja yang berpusat pada Kristus dan berdampak bagi lingkungan.</p>
                            </motion.div>
                            <motion.div variants={sejarahItemVariants} className="space-y-3">
                                <div className="w-12 h-1 bg-gmmi-navy rounded-full"></div>
                                <h4 className="font-bold text-gmmi-navy text-xl uppercase tracking-tighter">Misi Suci</h4>
                                <p className="text-sm text-neutral-500 font-medium">Mewartakan kabar baik melalui aksi pelayanan kasih yang holistik.</p>
                            </motion.div>
                        </div>
        ...
        const fetchData = async () => {
            ...
            try {
                const results = await Promise.allSettled([
                    getPrograms({}).catch(err => { console.error("Programs Error:", err); return []; }),
                    getWarta().catch(err => { console.error("Warta Error:", err); return null; }),
                    getPengumuman().catch(err => { console.error("Pengumuman Error:", err); return []; }),
                    getAllRenungan().catch(err => { console.error("Renungan Error:", err); return []; }),
                    getCarouselSlides().catch(err => { console.error("Carousel Error:", err); return []; }),
                    getKeuanganSummary().catch(err => { console.error("Finance Error:", err); return null; })
                ]);
                ...
                setRenungan(Array.isArray(renunganData) && renunganData.length > 0 ? renunganData[0] : null);
            ...
            <main className="container mx-auto px-6 py-24 space-y-32">
                    ...
                    <div className="lg:col-span-7 space-y-8">
                        ...
                        <section id="pewartaan">
                            <div className="flex items-center justify-between mb-6">
                                ...
                                <Link to="/pewartaan" className="text-sm font-bold text-gmmi-gold hover:text-gmmi-navy transition-colors flex items-center gap-1">
                                                ...
                                                <div className="flex h-full">
                                                    <div className="w-1/3 bg-gmmi-navy relative overflow-hidden">
                                                        ...
                                                        <div className="absolute top-4 left-4 flex flex-col items-center">
                                                            <span className="text-white font-black text-2xl">{new Date(item.tanggal_ibadah).getDate()}</span>
                                                            <span className="text-gmmi-gold text-[10px] font-black uppercase">{new Date(item.tanggal_ibadah).toLocaleString('id-ID', { month: 'short' })}</span>
                                                        </div>
                                                    </div>
                                                    <div className="w-2/3 p-6 flex flex-col justify-center space-y-2">
                                                        <div className="flex items-center gap-2">
                                                            <Chip size="sm" color="primary" variant="flat" className="text-[8px] h-5 font-black uppercase tracking-widest">{item.tempat_jemaat}</Chip>
                                                        </div>
                                                        <h4 className="text-lg font-bold text-gmmi-navy line-clamp-1">{item.judul}</h4>
                                                        <p className="text-xs text-neutral-500 line-clamp-2 italic">"{item.tema_khotbah}"</p>
                                                    </div>
                                                </div>
                                            </Card>
                                        </motion.div>
                                    ...
                                    >
                                        ...
                                        <div className="relative z-10 h-full p-10 flex flex-col justify-center max-w-md space-y-4">
                                            ...
                                            <h4 className="text-3xl font-bold text-white leading-tight">
                                                Informasi & Warta <br /> <span className="text-gmmi-gold">Keluarga Allah</span>
                                            </h4>
                                            <p className="text-slate-300 font-light text-sm leading-relaxed">
                                                Dapatkan informasi terbaru mengenai jadwal ibadah, kegiatan jemaat, dan berita sukacita lainnya di halaman pewartaan kami.
                                            </p>
                                        </div>
                                    </Card>
                                )}
                        ...
                        <section id="pengumuman">
                            <div className="flex items-center justify-between mb-6">
                                ...
                                <Link to="/pengumuman" className="text-sm font-bold text-gmmi-gold hover:text-gmmi-navy transition-colors flex items-center gap-1">
                                ...
                                {loading ? (
                                    ...
                                    announcements.map((item, i) => (
                                        ...
                                        >
                                            ...
                                            <div className="space-y-1">
                                                <p className="text-sm text-gmmi-navy font-medium leading-relaxed">{item.isi}</p>
                                                <p className="text-[10px] text-neutral-400 font-bold uppercase tracking-widest">
                                                    {new Date(item.tanggal || item.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'long' })}
                                                </p>
                                            </div>
                                        </motion.div>
                                    ))
                                    ...
                                    >
                                        <div className="relative z-10 space-y-4">
                                            <h4 className="text-2xl font-bold text-white">Belum ada pengumuman baru.</h4>
                                            <p className="text-slate-400 text-sm">Klik di sini untuk melihat arsip pengumuman sebelumnya.</p>
                                        </div>
                                    </Card>
                                )}
                            </div>
                        ...
                        >
                            ...
                            <div className="relative z-10">
                                <div className="mb-12">
                                    ...
                                    <h2 className="text-4xl font-bold leading-tight tracking-tight">
                                        Jadwal Ibadah<br />& Kegiatan
                                    </h2>
                                </div>

                                <div className="space-y-6">
                                    <p className="text-slate-400 text-lg font-light leading-relaxed">
                                        Temukan seluruh rangkaian agenda pelayanan, jadwal ibadah raya, hingga kegiatan kategorial dalam satu tempat.
                                    </p>
                            ...
                            <div className="relative z-10 pt-10 grid grid-cols-2 gap-4">
                                <div className="p-5 bg-white/5 rounded-3xl border border-white/5 backdrop-blur-sm">
                                    ...
                                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-300">Update</p>
                                    <p className="text-sm font-bold">Setiap Hari</p>
                                </div>
                                <div className="p-5 bg-white/5 rounded-3xl border border-white/5 backdrop-blur-sm">
                                    <MapPin className="w-6 h-6 text-gmmi-gold mb-3" />
                                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-300">Lokasi</p>
                            ...
                            <div className="relative z-10 flex flex-col h-full justify-between space-y-6">
                                ...
                                <div className="flex items-center justify-between mt-auto pt-4 border-t border-gray-50">
                                    <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">
                                        {renungan ? new Date(renungan.tanggal).toLocaleDateString('id-ID', { day: 'numeric', month: 'long' }) : "Segera Hadir"}
                                    </span>
                            ...
                            <div className="relative z-10 space-y-8">
                                <div>
                                    ...
                                    <h3 className="text-3xl font-bold text-white leading-tight">
                                        Transparansi <br /> <span className="text-gmmi-gold">Kas Gereja</span>
                                    </h3>
                                </div>

                                <div className="space-y-4">
                                    <div className="p-6 bg-white/5 rounded-3xl border border-white/10 backdrop-blur-sm">
                                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Saldo Saat Ini</p>
                                        <p className="text-3xl font-bold text-white">
                                            {financeSummary ?
                                                new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits: 0 }).format(financeSummary.balance)
                                                : 'Rp 0'}
                                        </p>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="p-4 bg-green-500/10 rounded-2xl border border-green-500/20">
                                            <p className="text-[10px] font-black uppercase tracking-widest text-green-400 mb-1">Pemasukan</p>
                                            <p className="text-sm font-bold text-white">
                                                {financeSummary ?
                                                    new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits: 0 }).format(financeSummary.totalIncome)
                                                    : 'Rp 0'}
                                            </p>
                                        </div>
                                        <div className="p-4 bg-red-500/10 rounded-2xl border border-red-500/20">
                                            <p className="text-[10px] font-black uppercase tracking-widest text-red-400 mb-1">Pengeluaran</p>
                                            <p className="text-sm font-bold text-white">
                                                {financeSummary ?
                                                    new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', currencyDisplay: 'symbol', maximumFractionDigits: 0 }).format(financeSummary.totalExpense)
                                                    : 'Rp 0'}
                                            </p>
                                        </div>
                                    </div>
                            ...
                            <div className="flex flex-col lg:flex-row items-center gap-16 relative z-10">
                                <div className="lg:w-1/2 space-y-8 text-center lg:text-left">
                                    ...
                                    <div className="space-y-6">
                                        <h2 className="text-5xl md:text-7xl font-bold text-gmmi-navy tracking-tighter leading-[0.9]">
                                            Program Kerja <br /> <span className="text-gmmi-gold">& Pelayanan</span>
                                        </h2>
                                        <p className="text-neutral-500 text-xl font-light leading-relaxed max-w-xl">
                                            Mewujudkan visi gereja melalui berbagai aksi nyata dalam pelayanan, persekutuan, dan pengembangan jemaat.
                                        </p>
                                    </div>
                                ...
                                <div className="lg:w-1/2 grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {loading ? (
                                        ...
                                        programs.map((prog, i) => (
                                            <div key={i} className="p-8 bg-neutral-50 rounded-[2.5rem] border border-neutral-100 group-hover:bg-white group-hover:shadow-xl transition-all duration-500 flex flex-col justify-between">
                                                <div>
                                                    <div className="text-gmmi-gold mb-4">
                                                        {prog.bidang === 'Pewartaan' ? <Target size={24} /> :
                                                            prog.bidang === 'Pelayanan' ? <Activity size={24} /> :
                                                                <Sparkles size={24} />}
                                                    </div>
                                                    <span className="text-gmmi-navy font-bold text-sm tracking-tight block mb-2">{prog.nama_program}</span>
                                                    <p className="text-xs text-neutral-500 line-clamp-2">{prog.jenis_kegiatan}</p>
                                                </div>
                                        ...
                                        [
                                            { label: "Pewartaan", icon: <Target className="w-6 h-6" /> },
                                            { label: "Pelayanan", icon: <Activity className="w-6 h-6" /> },
                                            { label: "Persekutuan", icon: <Users className="w-6 h-6" /> },
                                            { label: "Pendidikan", icon: <BookOpen className="w-6 h-6" /> }
                                        ].map((item, i) => (
                                            <div key={i} className="p-8 bg-neutral-50 rounded-[2.5rem] border border-neutral-100 group-hover:bg-white group-hover:shadow-xl transition-all duration-500">
                                                <div className="text-gmmi-gold mb-4">{item.icon}</div>
                                                <span className="text-gmmi-navy font-bold text-sm tracking-tight">{item.label}</span>
                                            </div>
                                        ))
                                    )}
                                </div>
                ...
                <section id="faq" className="space-y-16 py-10">
                    <div className="text-center space-y-4">
                        ...
                        <h2 className="text-5xl font-bold text-gmmi-navy tracking-tight">Informasi Penting</h2>
                    </div>

                    <div className="max-w-4xl mx-auto space-y-6">
                        {[
                            { q: "Kapan jadwal ibadah raya GMMI?", a: "Ibadah Raya kami diadakan setiap hari Minggu pukul 08:00 WIB dan 10:00 WIB. Kami juga menyediakan layanan live streaming bagi jemaat yang berhalangan hadir fisik." },
                            { q: "Bagaimana cara menjadi anggota jemaat?", a: "Anda dapat menghubungi sekretariat kami setelah ibadah selesai atau mengisi formulir pendaftaran jemaat melalui portal yang tersedia di website ini." },
                            { q: "Apakah ada kegiatan untuk anak-anak dan remaja?", a: "Tentu! Kami memiliki Sekolah Minggu untuk anak-anak (pukul 08:00 WIB) dan persekutuan remaja/pemuda setiap hari Sabtu pukul 18:00 WIB." }
                        ].map((item, i) => (
                            <motion.div
                                key={i}
                                initial={{ opacity: 0, x: -20 }}
                            ...
                            >
                                <Accordion
                                    ...
                                    sx={{
                                        ...
                                        '&.Mui-expanded': {
                                            ...
                                            backgroundColor: '#fff',
                                            boxShadow: '0 20px 40px -15px rgba(212,175,55,0.08)'
                                        },
                                        '&:hover': {
                                ...
                                >
                                    <AccordionSummary
                                        expandIcon={<div className="bg-gmmi-gold/10 p-2 rounded-xl text-gmmi-gold group-hover:bg-gmmi-gold group-hover:text-white transition-colors duration-300"><Plus className="w-5 h-5 scale-90" /></div>}
                                        sx={{
                                            px: 5,
                                            py: 2,
                                    ...
                                    >
                                        <Typography sx={{ fontWeight: 800, color: '#0f172a', fontSize: '1.25rem', tracking: '-0.02em' }}>
                                            {item.q}
                                        </Typography>
                                    </AccordionSummary>
                                    <AccordionDetails sx={{ px: 5, pb: 5, pt: 0 }}>
                                        <Typography sx={{ color: '#64748b', lineHeight: 2, fontSize: '1rem', fontWeight: 300 }}>
                                            {item.a}
                                        </Typography>
                                    </AccordionDetails>
                                </Accordion>
