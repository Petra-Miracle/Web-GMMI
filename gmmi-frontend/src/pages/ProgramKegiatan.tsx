                ...
                <Card className="p-3 rounded-[2.5rem] shadow-2xl border-none mb-20 max-w-5xl mx-auto bg-white/80 backdrop-blur-xl">
                    <div className="flex flex-col md:flex-row gap-4">
                        <Input
                            ...
                            startContent={<Search className="text-gmmi-gold ml-2" size={24} />}
                            value={searchTerm}
                        ...
                        <div className="flex items-center gap-4 px-8 bg-neutral-100 rounded-full shrink-0">
                            <span className="text-gmmi-navy font-black text-xs uppercase tracking-widest">{Object.keys(groupedPrograms).length} Bidang Terkait</span>
                        </div>
                    </div>
                </Card>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                    {loading ? (
                        Array(6).fill(0).map((_, i) => (
                            <Card key={i} className="p-10 rounded-[3rem] border border-neutral-100 h-[300px] flex flex-col justify-between" radius="lg">
                                    ...
                                    <div className="space-y-8 w-full text-left">
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <p className="text-neutral-400 text-sm font-light leading-relaxed">
                                                Klik untuk melihat detail seluruh rencana kerja di bidang {bidang.toLowerCase()}.
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="pt-8 flex items-center justify-between w-full">
                                        <div className="flex -space-x-3">
                                            {progs.slice(0, 3).map((_, i) => (
                                                <div key={i} className="w-10 h-10 rounded-full bg-neutral-100 border-2 border-white flex items-center justify-center text-gmmi-gold shadow-sm">
                                                    <Sparkles size={14} />
                                                </div>
                                            ))}
                                            {progs.length > 3 && (
                        ...
                        <div className="col-span-full text-center py-40">
                            ...
                            <h3 className="text-3xl font-bold text-gmmi-navy mb-4">Program tidak ditemukan</h3>
                            <p className="text-slate-400 max-w-md mx-auto text-lg font-light leading-relaxed">
                                Silakan cari dengan kata kunci lain atau lihat program kami yang tersedia.
                            </p>
                        </div>
                    )}
                </div>
            ...
            <section className="bg-neutral-50 py-32 overflow-hidden relative">
                ...
                <div className="container mx-auto px-6">
                    <div className="text-center mb-20 space-y-4">
                        <h2 className="text-4xl md:text-5xl font-bold text-gmmi-navy tracking-tight">Arah Pelayanan</h2>
                        <p className="text-slate-500 max-w-xl mx-auto font-light">Fokus utama pelayanan GMMI dalam mewujudkan gereja yang berdampak.</p>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                        {[
                            { title: "Pewartaan", desc: "Menyebarluaskan Firman Tuhan melalui berbagai kanal komunikasi.", color: "bg-blue-500", icon: <FileText size={20} /> },
                            { title: "Persekutuan", desc: "Membangun relasi antar jemaat yang berakar pada kasih Kristus.", color: "bg-purple-500", icon: <Users size={20} /> },
                            { title: "Pelayanan", desc: "Aksi nyata kepedulian sosial untuk sesama yang membutuhkan.", color: "bg-emerald-500", icon: <Activity size={20} /> },
                            { title: "Penatalayanan", desc: "Manajemen dan pengelolaan sumber daya gereja yang akuntabel.", color: "bg-gmmi-gold", icon: <LayoutDashboard size={20} /> }
                        ].map((bid, i) => (
                            <Card key={i} className="p-10 rounded-[3rem] border-none shadow-xl hover:shadow-2xl transition-all duration-500 group">
                                <div className={`w-12 h-12 ${bid.color} rounded-2xl mb-8 flex items-center justify-center text-white shadow-lg`}>
                                    {bid.icon}
                                </div>
                                <h4 className="text-2xl font-bold text-gmmi-navy mb-4">{bid.title}</h4>
                                <p className="text-slate-500 font-light leading-relaxed">{bid.desc}</p>
                            </Card>
                        ))}
                    </div>
                </div>
