...
import { createProgram, getPrograms, exportExcel, exportWord, deleteProgram } from '../../services/program.service';
...
const SuperAdminProgram: React.FC = () => {
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        ...
        setFormData(prev => ({
            ...prev,
            [name]: name === 'volume' || name === 'rencana_biaya' ? Number(value) : value
        }));
    };
    ...
    const handleSelectChange = (name: string, value: string) => {
        setFormData(prev => ({
            ...
            ...(name === 'bidang' && value !== 'Penatalayanan' ? { sub_bidang: '' } : {})
        }));
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const errorMessage = error.response?.data?.message || 'Gagal menambahkan program';
            setToast({
                open: true,
    ...
    const handleExportExcel = async () => {
        try {
            ...
            a.href = url;
            a.download = filterBidang ? `Program_Kerja_${filterBidang}_${currentYear}.xlsx` : `Program_Kerja_GMMI_${currentYear}.xlsx`;
            a.click();
            setToast({
                open: true,
    ...
    const handleExportWord = async () => {
        try {
            ...
            const a = document.createElement('a');
            a.href = url;
            a.download = filterBidang ? `Program_Kerja_${filterBidang}_${currentYear}.docx` : `Program_Kerja_GMMI_${currentYear}.docx`;
            a.click();
            setToast({
                open: true,
    ...
    return (
        <div className="p-6 space-y-8 max-w-7xl mx-auto">
            <div className="flex justify-between items-center">
                <h1 className="text-3xl font-bold text-gray-800">Manajemen Program & Kegiatan Gereja</h1>
            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                {formData.bidang === 'Penatalayanan' && (
                                    <Select
                                        ...
                                        selectedKeys={formData.sub_bidang ? [formData.sub_bidang] : []}
                                        onChange={(e) => handleSelectChange('sub_bidang', e.target.value)}
                                        isRequired
                                    >
                                        <SelectItem key="Pelayanan Umum" textValue="Pelayanan Umum">Pelayanan Umum</SelectItem>
                                        <SelectItem key="Pelayanan Khusus" textValue="Pelayanan Khusus">Pelayanan Khusus</SelectItem>
                                    </Select>
                                )}
            ...
            <Card className="shadow-lg border border-gray-100">
                <CardBody className="p-0 overflow-hidden">
                    <div className="p-6 bg-gray-50 border-b border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4">
                        <div className="flex items-center gap-3 w-full md:max-w-md">
                            ...
                            <Select
                                ...
                                onChange={(e) => setFilterBidang(e.target.value === 'all' ? '' : e.target.value)}
                                className="w-full"
                            >
                                {['all', ...BIDANG_OPTIONS].map(opt => (
                                    <SelectItem key={opt} textValue={opt === 'all' ? "Semua Bidang" : opt}>
                    ...
                    >
                        ...
                        <TableBody
                            emptyContent={loading ? null : "Tidak ada data program ditemukan."}
                            isLoading={loading}
                                ...
                                <TableRow key={item.id}>
                                    <TableCell>
                                        <div className="flex flex-col gap-1">
                                            <span className="font-bold text-blue-900">{item.bidang}</span>
                                            {item.sub_bidang && (
                                                <Chip size="sm" variant="flat" color="primary">{item.sub_bidang}</Chip>
                                            )}
                                        </div>
                                    </TableCell>
                                    <TableCell>
                                        <div className="font-semibold">{item.nama_program}</div>
                                        <div className="text-xs text-gray-400 font-light">Vol: {item.volume}</div>
                                    </TableCell>
                                    <TableCell>
                                        <div className="max-w-xs text-sm text-gray-600 line-clamp-2" title={item.jenis_kegiatan}>
                                            {item.jenis_kegiatan}
                                        </div>
                                    </TableCell>
                                    ...
                                    <TableCell>
                                        <div className="max-w-xs truncate text-xs text-gray-500 italic" title={item.keterangan}>
                                            {item.keterangan || "-"}
                                        </div>
                                    </TableCell>
                                    <TableCell>
            ...
            >
                <Alert onClose={handleCloseToast} severity={toast.severity} sx={{ width: '100%', borderRadius: '12px', fontWeight: 'bold' }}>
                    {toast.message}
                </Alert>
            </Snackbar>
        </div>
